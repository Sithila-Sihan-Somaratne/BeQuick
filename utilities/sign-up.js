...
router.post('/sign-up', (req, res) => {
    ...
    const sql = 'INSERT INTO user (userName, userEmail, userContact, userPwd, verificationPin, isVerified) VALUES (?, ?, ?, ?, ?, ?)';
...
router.post('/verify-pin', (req, res) => {
    ...
    const sql = 'SELECT * FROM user WHERE userName = ? AND verificationPin = ?';
    ...
    connection.query(sql, [userName, verificationPin], (err, results) => {
        if (err) {
            ...
            const updateSql = 'UPDATE user SET isVerified = 1 WHERE userName = ?';
